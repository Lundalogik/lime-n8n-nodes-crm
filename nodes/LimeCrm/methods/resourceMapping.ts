...
function getPropertyLength(property: LimetypeProperty): string {
	const length =
		property.length !== undefined && property.length < 10_000 ? property.length : 'max';
	return ['string', 'text', 'link'].includes(property.type) ? `(${length})` : '';
}
...
export function parseResourceMapperFields(
	...
	const propertiesInput = context.getNodeParameter(inputName, i) as ResourceMapperValue;

	const parsedData = propertiesInput.value as IDataObject;

	return serializeResourceMapperValues(parsedData, propertiesInput.schema) as IDataObject;
}
...
export async function getRelationLookupMappingColumns(
	...
	const relationProperties = propertiesResponse.data.filter(
		(property) => property.type === 'belongsto' || property.type === 'hasone',
	);
