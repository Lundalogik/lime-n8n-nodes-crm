...
describe('LimeCrmNode execute — metadata resource', () => {
    ...
    it('getSingleLimetype fetches the requested limetype', async () => {
        transportMock.getLimetype.mockResolvedValue(
            ok({ name: 'person', properties: [] }) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'metadata',
    ...
    it('getSingleFileMetadata fetches by file id', async () => {
        transportMock.getFileMetadata.mockResolvedValue(
            ok({ id: 'f1', filename: 'cv.pdf' }) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'metadata',
    ...
    it('getSingleFileMetadata fetches via limeobject and property', async () => {
        transportMock.getFileMetadataByLimeobject.mockResolvedValue(
            ok({ id: 'f1' }) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'metadata',
...
describe('LimeCrmNode execute — data resource', () => {
    it('createSingleObject creates from JSON input', async () => {
        ...
        transportMock.createLimeobject.mockResolvedValue(
            ok({ _id: 1, name: 'Jane' }) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'data',
        ...
        expect(transportMock.createLimeobject).toHaveBeenCalledWith(
            ctx,
            'person',
            { name: 'Jane' }
        );
    ...
    it('createSingleObject uploads binary file properties before creating', async () => {
        ...
        transportMock.createFile.mockResolvedValue(
            ok({ id: 'file_9' }) as never
        );
        transportMock.createLimeobject.mockResolvedValue(
            ok({ _id: 1, document: 'file_9' }) as never
        );
        transportMock.getFileMetadata.mockResolvedValue(
    ...
    it('getSingleObject fetches an object by id', async () => {
        transportMock.getLimeobject.mockResolvedValue(
            ok({ _id: '5', name: 'Acme' }) as never
        );
        transportMock.getProperties.mockResolvedValue(ok([]) as never);
        const ctx = makeNodeExecuteContext({
            resource: 'data',
        ...
        expect(transportMock.getLimeobject).toHaveBeenCalledWith(
            ctx,
            'company',
            '5'
        );
        expect(out[0]).toEqual([
            { json: { _id: '5', name: 'Acme' }, binary: {} },
        ]);
    });

    it('updateSingleObject updates from JSON input', async () => {
        transportMock.getProperties.mockResolvedValue(ok([]) as never);
        transportMock.updateLimeobject.mockResolvedValue(
            ok({ _id: '5', name: 'New name' }) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'data',
        ...
        expect(transportMock.updateLimeobject).toHaveBeenCalledWith(
            ctx,
            'company',
            '5',
            { name: 'New name' }
        ...
        expect(out[0]).toEqual([
            { json: { _id: '5', name: 'New name' }, binary: {} },
        ]);
    });
    ...
    it('deleteSingleObject deletes an object by id', async () => {
        ...
        expect(transportMock.deleteLimeobject).toHaveBeenCalledWith(
            ctx,
            'company',
            '5'
        );
    ...
    it('getSingleFile fetches via limeobject and property', async () => {
        const binary = { data: 'YmluYXJ5', mimeType: 'application/pdf' };
        transportMock.getFileContentByLimetype.mockResolvedValue(
            ok(binary) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'data',
    ...
    it('getSingleFile passes an API error envelope through as item json', async () => {
        transportMock.getFileContent.mockResolvedValue(
            apiError('not found') as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'data',
            operation: 'getSingleFile',
    ...
    it('getSingleFile throws for an unknown source', async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow(
            'The source "bogus" is not supported!'
        );
    });
});
...
describe('LimeCrmNode execute — bulk import operations', () => {
    ...
    it('runs a single job for the whole input batch and returns one summary item', async () => {
        ...
        expect(transportMock.uploadBulkImportData).toHaveBeenCalledWith(
            ctx,
            'job1',
            [
                { values: { name: 'obj0' } },
    ...
    it('wraps every entry of a top-level array and reads the field once', async () => {
        ...
        expect(
            transportMock.createBulkImportJob.mock.calls[0][1].properties
        ).toEqual(['name', 'maps_latitude']);
    });
...
describe('LimeCrmNode execute — admin resource', () => {
    it('getSingleUser fetches by user id', async () => {
        ...
        expect(transportMock.fetchSingleUserById).toHaveBeenCalledWith(
            ctx,
            'u1',
            true
        );
    ...
    it('getSingleUser fetches by limeobject id', async () => {
        transportMock.fetchSingleUserByLimeobjectId.mockResolvedValue(
            ok({ id: 'u1' }) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'admin',
        ...
        expect(
            transportMock.fetchSingleUserByLimeobjectId
        ).toHaveBeenCalledWith(ctx, '42', false);
        expect(out[0]).toEqual([{ json: { id: 'u1' } }]);
    });

    it('getManyUsers returns one item per user', async () => {
        transportMock.fetchManyUsers.mockResolvedValue(
            ok([{ id: 'u1' }, { id: 'u2' }]) as never
        );
        const ctx = makeNodeExecuteContext({
            resource: 'admin',
...
describe('LimeCrmNode execute — dispatch and error handling', () => {
    ...
    it('executes the operation once per input item', async () => {
        ...
        expect(transportMock.getLimeobject).toHaveBeenNthCalledWith(
            1,
            ctx,
            'person',
            'id0'
